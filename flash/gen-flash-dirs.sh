...
#   2. Run ptool to generate fresh rawprogram*.xml, patch*.xml, gpt_*.bin, zeros_*.bin
...
for i in $(seq 0 $((BOARD_COUNT - 1))); do
    ...
    for storage in "${SUPPORTED_STORAGES[@]}"; do
        ...
        # 5. Wire OS image filenames in rawprogram XMLs.
        #    ptool generates rawprogram XMLs with filename="" for OS partitions.
        #    Patch them to point at the correct files in this flash dir.
        case "$storage" in
            nvme|ufs|emmc)
            ...
            spinor)
                for xml in "${STORAGE_DIR}"/rawprogram*.xml; do
                    [[ -f "$xml" ]] || continue
                    xmlstarlet ed -L \
                        -u "//program[@label='dtb_a' and @filename='']/@filename" \
                            -v "dtb.bin" \
                        -u "//program[@label='dtb_b' and @filename='']/@filename" \
                            -v "dtb.bin" \
                        "$xml"
