...
def create_pr_body(base_branch: str, upstream_tag: str, normalized_version: str, promoted_upstream_tag: str) -> str:
    return f"""
...
- PR branch: `promotion-pr/{normalized_version}-1`
- Changelog update: `{normalized_version}-1` (UNRELEASED)
...
gitGraph BT:
  ...
  commit
  commit
  branch promotion-pr/{normalized_version}-1 order: 2
...
def main():
    ...
    pr_body = create_pr_body(args.base_branch, args.upstream_tag, args.normalized_version, args.promoted_upstream_tag)
    ...
    pr_creation_command = f"gh pr create --title '{pr_title}' --body-file promotion_pr_body.md --base {args.base_branch} --head promotion-pr/{args.normalized_version}-1"
