...
# ------------------------------------------------------------------------------
# Description:
#   Writes VolatileVars.bin (see gen_volatile_vars.py) into the root of an
...
Notes:
  ...
  - --config forwards a JSON config to gen_volatile_vars.py to add/override
    entries on top of its built-in defaults (see gen_volatile_vars.py).
EOF
}
